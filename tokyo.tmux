...
main() {
  local theme
  theme="$(get_tmux_option "@tokyo" "dark")"
  ...
  # variables.
  # shellcheck source=tokyo-frappe.tmuxtheme
  source /dev/stdin <<<"$(sed -e "/^[^#].*=/s/^/local /" "${PLUGIN_DIR}/tokyo-${theme}.tmuxtheme")"
  ...
  # NOTE: Checking for the value of @tokyo_window_tabs_enabled
  local wt_enabled
  wt_enabled="$(get_tmux_option "@tokyo_window_tabs_enabled" "off")"
  readonly wt_enabled

  local right_separator
  right_separator="$(get_tmux_option "@tokyo_right_separator" "")"
  readonly right_separator

  local left_separator
  left_separator="$(get_tmux_option "@tokyo_left_separator" "")"
  readonly left_separator

  local user
  user="$(get_tmux_option "@tokyo_user" "off")"
  readonly user

  local host
  host="$(get_tmux_option "@tokyo_host" "off")"
  readonly host

  local date_time
  date_time="$(get_tmux_option "@tokyo_date_time" "off")"
  readonly date_time
 ...
 #setw -g window-status-current-format "#[fg=colour232,bg=$thm_orange] #I #[fg=colour255,bg=colour237] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
  ...
  # NOTE: With the @tokyo_window_tabs_enabled set to on, we're going to
